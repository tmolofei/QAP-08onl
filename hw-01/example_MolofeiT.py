...
#
# This program should not start, and if it suddenly starts and works, then some
# kind of magic has happened.
# Basically, because this is not a whole program, but only a piece of it.


FILENAME = 'file.txt'
parser = argparse.ArgumentParser(description='''Find logs according to the
                                given parameters. By default it prints
                                the first 300 symbols''')


class entries_count:
    def Find_entries(
                    self, log_entries: dict, text: str, unwanted: str,
                    date: str) -> dict:
        if date:
            if date.startswith('..'):
            ...
            for line in log:
                ...
                try:
                    date = datetime.strptime(
                                    possible_date, '%Y-%m-%d %H:%M:%S.%f')
                    # 2022-01-13 12:34:20.517
                    entries[date] = line
                except ValueError:
                    if date:
                        entries[date] += line
